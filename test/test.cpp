...
public:
  ...
  [[nodiscard]] std::optional<Vault::HttpResponse>
  get(const Vault::Url &url, const Vault::Token &token,
      const Vault::Namespace &ns) const override {
    return response_;
  }
  [[nodiscard]] std::optional<Vault::HttpResponse>
  post(const Vault::Url &url, const Vault::Token &token,
       const Vault::Namespace &ns, const std::string &value) const override {
    return response_;
  }
  [[nodiscard]] std::optional<Vault::HttpResponse>
  del(const Vault::Url &url, const Vault::Token &token,
      const Vault::Namespace &ns) const override {
    return response_;
  }
  [[nodiscard]] std::optional<Vault::HttpResponse>
  list(const Vault::Url &url, const Vault::Token &token,
       const Vault::Namespace &ns) const override {
    return response_;
  }
...
public:
  MockVaultClient(Vault::Config &config,                       // NOLINT
                  Vault::AuthenticationStrategy &authStrategy, // NOLINT
                  const MockHttpClient &mockHttpClient)
      : Vault::Client(config, authStrategy), mockHttpClient_(mockHttpClient) {}
...
TEST_CASE("VaultClient#getUrl") {
  auto strategy = SuccessfulAuth();
  auto vaultClient = Vault::Client(config, strategy);
  REQUIRE(vaultClient.getUrl("/base", Vault::Path{"/path"}).value() ==
          "https://localhost:8200/base/path");
}

TEST_CASE("VaultClient#getUrl tls false") {
  auto strategy = SuccessfulAuth();
  auto vaultClient = Vault::Client(
      Vault::ConfigBuilder().withTlsEnabled(false).build(), strategy);
  REQUIRE(vaultClient.getUrl("/base", Vault::Path{"/path"}).value() ==
          "http://localhost:8200/base/path");
}
...
TEST_CASE("VaultClient#getNamespace") {
  auto strategy = SuccessfulAuth();
  auto vaultClient = Vault::Client(
      Vault::ConfigBuilder().withNamespace(Vault::Namespace{"ns"}).build(),
      strategy);
...
TEST_CASE("HttpClient#is_success when status code not 200") {
  auto response = std::optional<Vault::HttpResponse>(
      {Vault::HttpResponseStatusCode{403},
       Vault::HttpResponseUrl{"http://example.com"},
       Vault::HttpResponseBodyString{"Permission Denied"}});
  REQUIRE(!Vault::HttpClient::is_success(response));
}

TEST_CASE("HttpClient#is_success when status 200") {
  auto response = std::optional<Vault::HttpResponse>(
      {Vault::HttpResponseStatusCode{200},
       Vault::HttpResponseUrl{"http://example.com"},
...
TEST_CASE("MockHttpClient#return mocked response") {
  ...
  REQUIRE(httpClient.get(Vault::Url("/test"), Vault::Token("foo"),
                         Vault::Namespace("bar")) == std::nullopt);
  REQUIRE(httpClient.post(Vault::Url("/test"), Vault::Token("foo"),
                          Vault::Namespace("bar"), "baz") == std::nullopt);
  REQUIRE(httpClient.del(Vault::Url("/test"), Vault::Token("foo"),
                         Vault::Namespace("bar")) == std::nullopt);
  REQUIRE(httpClient.list(Vault::Url("/test"), Vault::Token("foo"),
                          Vault::Namespace("bar")) == std::nullopt);

  auto resp =
      Vault::HttpResponse{Vault::HttpResponseStatusCode{200},
                          Vault::HttpResponseUrl{"http://example.com/test"},
                          Vault::HttpResponseBodyString("success")};
  httpClient.SetResponse(resp);
  auto getResp = httpClient.get(Vault::Url("/test"), Vault::Token("foo"),
                                Vault::Namespace("bar"));
  ...
  auto postResp = httpClient.post(Vault::Url("/test"), Vault::Token("foo"),
                                  Vault::Namespace("bar"), "baz");
  ...
  auto delResp = httpClient.del(Vault::Url("/test"), Vault::Token("foo"),
                                Vault::Namespace("bar"));
  ...
  auto listResp = httpClient.list(Vault::Url("/test"), Vault::Token("foo"),
                                  Vault::Namespace("bar"));
