...
std::optional<std::string> Vault::Sys::Mfa::PingId::configure(const Path &path, const Parameters &parameters) {
  return Vault::Sys::Mfa::configure(client_, getUrl(path), parameters);
}

std::optional<std::string> Vault::Sys::Mfa::PingId::del(const Path &path, const Parameters &parameters) {
