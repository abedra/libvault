...
std::optional<std::string> Vault::Sys::Auth::enable(const Path &path, const Parameters &parameters) const {
...
std::optional<std::string> Vault::Sys::Auth::readTuning(const Path &path) const {
  return HttpConsumer::get(client_, getUrl(Vault::Path{path + "/tune"}));
}

std::optional<std::string> Vault::Sys::Auth::tune(const Path &path, const Parameters &parameters) const {
