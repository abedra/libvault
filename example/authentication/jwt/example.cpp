...
Vault::Jwt makeJwt(const std::filesystem::path &publicKeyPath, const std::filesystem::path &privateKeyPath) {
  ...
  auto token = jwt::create()
      ...
      .sign(jwt::algorithm::rs256(publicKeyString, privateKeyString, "", ""));

  return Vault::Jwt{token};
}

Vault::Client setup(const Vault::Client &rootClient, const std::filesystem::path &publicKeyPath, const std::filesystem::path &privateKeyPath) {
...
int main(void) {
  ...
  if (!rootTokenEnv) {
    std::cout << "The VAULT_TOKEN environment variable must be set" << std::endl;
    exit(-1);
  }
  Vault::Token rootToken{rootTokenEnv};
  Vault::Client rootClient = getRootClient(rootToken);
  Vault::Client jwtClient = setup(rootClient, std::filesystem::path{"public.pem"}, std::filesystem::path{"private.pem"});
